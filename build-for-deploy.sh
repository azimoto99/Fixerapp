...
# Script to prepare for deployment with proper dependency workarounds
...
# Copy all source files to the deploy-build directory
cp -r server shared client deploy-build/
cp vite.config.ts tsconfig.json tailwind.config.ts deploy-build/

# Create a deployment-specific package.json without Expo dependencies
cat > deploy-build/package.json << EOF
{
...
# Move to the deploy build directory
cd deploy-build

# Create a modified vite.config.ts that doesn't depend on Expo
