...
export async function geocodeAddress(address: string): Promise<GeocodingResult> {
  try {
    ...
    // This is a free service with usage limits, so in a production app
    // you might want to use a commercial geocoding service instead
    ...
    if (!data || data.length === 0) {
      return {
        ...
        longitude: 0,
        success: false,
        error: 'Location not found. Please try a different address or postal code.'
      };
    }
