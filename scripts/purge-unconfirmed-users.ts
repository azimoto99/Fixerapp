...
(async () => {
  ...
  try {
    const unconfirmed = await storage.getAllUsers();
    const unconfirmedFiltered = unconfirmed.filter(u => !u.emailVerified);
    const toPurge = unconfirmedFiltered.filter(u => new Date(u.createdAt) < cutoff);
