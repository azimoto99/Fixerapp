...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add debugging routes for Google OAuth setup
  app.get('/auth-test', (req, res) => {
    res.send(`
      <h1>Google Authentication Test</h1>
      <p>Use the button below to test Google authentication:</p>
      <a href="/auth/google" style="display: inline-block; background-color: #4285F4; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">
        Login with Google
      </a>
      <div style="margin-top: 20px;">
        ...
        <pre>
          ...
          Google Client Secret Configured: ${process.env.GOOGLE_CLIENT_SECRET ? 'Yes' : 'No'}
        </pre>
      </div>
    `);
  });
  
  // Add route to test regular registration
